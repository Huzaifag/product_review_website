...
$backupFile = __DIR__ . '/products_backup.json';

// ═══════════════════════════════════════════════════════════════
//  STEP 1 — Fetch products
// ═══════════════════════════════════════════════════════════════
echo "\n📦 Step 1/3 — Fetching products...\n";

$rows  = $pdo->query("SELECT id, name, description FROM products")
...
$update = $pdo->prepare("UPDATE products SET name = ?, description = ? WHERE id = ?");
...
foreach ($rows as $row) {
    ...
    if ($translatedName === null) {
        ...
        $translatedName = $name; // keep original
        $failed++;
    }

    // ── Translate description ───────────────────────────────────
    $translatedDesc = googleTranslate($desc, 'en', 'de');
    if ($translatedDesc === null) {
        echo "   ⚠️  Desc FAILED [id=$id]\n";
        $translatedDesc = $desc; // keep original
        $failed++;
    }
    ...
    if (!$dryRun) {
        $update->execute([$translatedName, $translatedDesc, $id]);
    }
