    ...
    @if ($canSeeDetails)
                ...
                <div class="right-col">
                    ...
                    <div class="tab-bar">
                        <button class="tab-btn active" onclick="switchTab(this,'tab-concerns')">
                            ...
                            {{ strtoupper(d_trans('Ingredient Concerns')) }}
                            @if ($product->ingredientLibraries->count() > 0)
                                {{-- <span class="tab-count-badge">{{ $product->ingredientLibraries->count() }}</span> --}}
                            @endif
                        </button>
                        <button class="tab-btn" onclick="switchTab(this,'tab-snap')"><i class="fas fa-microscope"></i>
                            {{ strtoupper(d_trans('Lab Snapshot')) }}</button>
                        <button class="tab-btn" onclick="switchTab(this,'tab-full')"><i class="fas fa-file-alt"></i>
                            {{ strtoupper(d_trans('Full Details')) }}</button>
                        <button class="tab-btn" onclick="switchTab(this,'tab-cert')"><i class="fas fa-certificate"></i>
                            {{ strtoupper(d_trans('Certifications')) }}</button>
                    </div>
                    ...
                    {{-- Ingredient Library tab (pivot-based accordion) --}}
                    <div class="tab-panel active" id="tab-concerns">
                        @push('styles')
                            <style>
                                ...
                                .ilacc-trigger:hover {
                                    background: rgba(24, 40, 26, .03);
                                }

                                .ilacc-trigger.il-open {
                                    background: rgba(24, 40, 26, .04);
                                }
                                ...
                                .ilacc-trigger.il-open .ilacc-chevron {
                                    background: var(--gd);
                                    border-color: var(--gd);
                                    color: #fff;
                                }

                                .ilacc-trigger.il-open .ilacc-chevron svg {
                                ...
                                .ilacc-fn-pill {
                                    ...
                                    color: var(--gd);
                                    background: #fff;
                                    border: 1px solid rgba(24, 40, 26, .18);
                                    border-radius: 999px;
                                    padding: 4px 13px;
                                    letter-spacing: .01em;
                        ...
                        @php
                            $linkedLibs = $product->ingredientLibraries;
                            $avoidCount = $linkedLibs->where('severity', 'avoid')->count();
                            $concernCount = $linkedLibs->where('severity', 'concern')->count();
                            $cautionCount = $linkedLibs->where('severity', 'caution')->count();
                            $safeCount = $linkedLibs->whereIn('severity', ['none', null])->count();
                        @endphp

                        <div class="ilacc-wrap">
                            @if ($linkedLibs->count() > 0)
                                {{-- Summary strip --}}
                                <div class="ilacc-summary">
                                    @if ($avoidCount)
                                        <div class="concerns-summary-chip chip-high">
                                            <span class="chip-dot"></span>{{ $avoidCount }} {{ d_trans('Avoid') }}
                                        </div>
                                    @endif
                                    @if ($concernCount)
                                        <div class="concerns-summary-chip chip-medium">
                                            <span class="chip-dot"></span>{{ $concernCount }} {{ d_trans('Concern') }}
                                        </div>
                                    @endif
                                    @if ($cautionCount)
                                        <div class="concerns-summary-chip" style="background:#fef9c3;color:#854d0e;">
                                            <span class="chip-dot"></span>{{ $cautionCount }} {{ d_trans('Caution') }}
                                        </div>
                                    @endif
                                    @if ($safeCount)
                                        <div class="concerns-summary-chip chip-low">
                                            <span class="chip-dot"></span>{{ $safeCount }} {{ d_trans('Safe') }}
                                        </div>
                                    @endif
                                    <span
                                        class="ilacc-summary-note">{{ d_trans('Click an ingredient for more information.') }}</span>
                                </div>

                                {{-- Accordion items --}}
                                @foreach ($linkedLibs as $libIdx => $lib)
                                    @php
                                        $sev = $lib->severity ?? 'none';
                                        $stripeClass = 'il-stripe-' . ($sev ?: 'none');
                                        $badgeClass = 'il-badge-' . ($sev ?: 'none');
                                        $badgeLabel = match ($sev) {
                                            'avoid' => d_trans('Avoid'),
                                            'concern' => d_trans('Concern'),
                                            'caution' => d_trans('Caution'),
                                            default => d_trans('Safe'),
                                        };
                                        $fnPreview = !empty($lib->functions)
                                            ? \Illuminate\Support\Str::limit(implode(', ', $lib->functions), 60)
                                            : null;
                                    @endphp
                                    <div class="ilacc-item">
                                        <button class="ilacc-trigger" type="button"
                                            onclick="ilAccToggle(this, 'il-body-{{ $libIdx }}')">
                                            <span class="ilacc-stripe {{ $stripeClass }}"></span>
                                            <span class="ilacc-trigger-main">
                                                <span class="ilacc-name">{{ $lib->name }}</span>
                                                @if ($fnPreview)
                                                    <span class="ilacc-meta">{{ $fnPreview }}</span>
                                                @endif
                                            </span>
                                            <span class="ilacc-right">
                                                <span class="ilacc-badge {{ $badgeClass }}">{{ $badgeLabel }}</span>
                                                <span class="ilacc-chevron">
                                        ...
                                        <div class="ilacc-body" id="il-body-{{ $libIdx }}">

                                            {{-- Functions --}}
                                            @if (!empty($lib->functions))
                                                <div class="ilacc-fn">
                                                    <div class="ilacc-fn-lbl">{{ d_trans('Function(s)') }}</div>
                                                    <div class="ilacc-fn-pills">
                                                        @foreach ($lib->functions as $fn)
                                                            <span class="ilacc-fn-pill">{{ $fn }}</span>
                                                        @endforeach
                                                    </div>
                                                </div>
                                            @endif
                                            ...
                                            {{-- Concerns --}}
                                            @if (!empty($lib->concerns) || $lib->concern_description)
                                                <div class="ilacc-fn" style="padding-bottom:0;">
                                                    <div class="ilacc-fn-lbl">{{ d_trans('Concerns') }}</div>
                                                </div>
                                                @if (!empty($lib->concerns))
                                                    @foreach ($lib->concerns as $c)
                                                        @if (!empty($c['concern_text']))
                                                            ...
                                                            @endphp
                                                            <div class="ilacc-concern-row">
                                                                @if (!empty($c['category']))
                                                                    <span
                                                                        class="ilacc-concern-cat">{{ $c['category'] }}:</span>
                                                                @endif
                                                                <div>
                                                                    <div class="ilacc-concern-text">{{ $cleanText }}
                                                                    </div>
                                                                    @if (!empty($c['reference_org']))
                                                                        <div class="ilacc-concern-ref">
                                                                            {{ $c['reference_org'] }}</div>
                                                                    @endif
                                                                </div>
                                                            </div>
                                                ...
                                                @else
                                                    <div class="ilacc-concern-row">
                                                        <div class="ilacc-concern-text">{{ $lib->concern_description }}
                                                        </div>
                                                    </div>
                                                @endif
                                            @endif
                                ...
                                @endforeach
                            @else
                                <div class="ilacc-empty">
                                    <div class="ilacc-empty-icon"><i class="fas fa-flask"></i></div>
                                    <div class="ilacc-empty-title">{{ d_trans('No Ingredients Linked') }}</div>
                                    <div class="ilacc-empty-sub">
                                        {{ d_trans('No ingredient library entries have been linked to this product yet.') }}
                                    </div>
                                </div>
                            @endif
                        </div>
        ...
        <div class="hsi-grid mt-4">
                ...
                <div class="hsi-scale-wrap">
                    <div class="hsi-scale-inner">
                        <div class="hsi-scale-left">
                            <img src="{{ asset('images/oko/stamp-verification.webp') }}" class="hsi-mini-stamp"
                                alt="OKO">
            ...
            <div class="hsi-card">
                ...
                <div class="hsi-verify-head">
                    <img src="{{ asset('images/oko/stamp-verification.webp') }}" class="hsi-verify-stamp"
                        alt="OKO Verified">
                    <div>
                ...
                <div class="hsi-approach">
                    <div class="hsi-approach-title">{{ d_trans('A Product Qualifies for OKO Verification If It:') }}
                    </div>
                    <ul class="hsi-criteria">
                        <li>{{ d_trans('Achieves an overall hazard score that meets OKO\'s established safety threshold') }}
                        </li>
                        <li>{{ d_trans('Contains no ingredients appearing on OKO\'s Restricted Substances list') }}</li>
                        <li>{{ d_trans('Provides full ingredient disclosure, including all fragrance and preservative components') }}
                        </li>
                    </ul>
                </div>
            </div>
        ...
        @push('scripts')
            <script>
                ...
                function ilAccToggle(trigger, bodyId) {
                    const body = document.getElementById(bodyId);
                    const isOpen = body.classList.contains('il-open');
                    body.classList.toggle('il-open', !isOpen);
                    trigger.classList.toggle('il-open', !isOpen);
                }
