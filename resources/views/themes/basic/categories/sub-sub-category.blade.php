...
@section('title', $subSubCategory->trans->title ?? $subSubCategory->trans->title)
@section('header_title', $subSubCategory->trans->title ?? $subSubCategory->trans->title)
@section('description', $subSubCategory->trans->description)
@section('keywords', $subSubCategory->trans->keywords)
@section('breadcrumbs', Breadcrumbs::render('categories.sub-sub-category', $subSubCategory))
@section('breadcrumbs_schema', Breadcrumbs::view('breadcrumbs::json-ld', 'categories.sub-sub-category',
    $subSubCategory))
                ...
                'grid_title' =>
                    collect(request()->query())->except('page')->count() > 0
                        ? d_trans('Your search results for the ":name" category', [
                            'name' => strtolower($subSubCategory->trans->name),
                        ])
