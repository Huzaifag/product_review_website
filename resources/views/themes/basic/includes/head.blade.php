@php
    ...
    $ogImage = $__env->yieldContent('og_image')
        ...
        : (config('settings.seo.og_image') ? asset(config('settings.seo.og_image')) : asset(config('theme.settings.general.social_image')));
@endphp
<meta charset="UTF-8">
...
@else
    @if(config('settings.seo.robots_txt'))
        <meta name="robots" content="{{ config('settings.seo.robots_txt') }}">
    @endif
@endif
@if(config('settings.seo.meta_author'))
    <meta name="author" content="{{ config('settings.seo.meta_author') }}">
@endif
...
@if(config('settings.seo.twitter_username'))
<meta name="twitter:site" content="{{ config('settings.seo.twitter_username') }}">
@endif
<meta name="twitter:title" content="{!! $title !!}">
<meta name="twitter:description" content="{{ $description }}">
<meta name="twitter:image:src" content="{{ $ogImage }}">
<title>{!! $title !!}</title>
<link rel="canonical" href="{{ config('settings.seo.canonical_url') ?: url()->current() }}" />
...
@endif
@if(config('settings.seo.google_analytics_id'))
<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id={{ config('settings.seo.google_analytics_id') }}"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', '{{ config('settings.seo.google_analytics_id') }}');
</script>
@endif
