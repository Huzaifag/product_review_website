...
@section('header_title', d_trans('Subscription'))
@section('breadcrumbs', Breadcrumbs::render('business.subscription'))
@section('content')
    <div class="card mb-4">
        <div class="card-body p-4">
            @if ($authBusinessOwner->isSubscribed())
                @php
                ...
                <div class="row align-items-center g-4 p-2">
                    <div class="col-12 col-lg">
                        <h3 class="mb-3">
                            {{ d_trans(':plan_name (:plan_interval)', [
                                'plan_name' => $subscription->plan->trans->name,
                    ...
                    <div class="col-12 col-lg-auto">
                        <div class="row row-cols-auto g-3">
                            @if ($subscription->isAboutToExpire() || $subscription->isExpired())
                                <div class="col">
                                    @if ($subscription->plan->isActive())
                                        <form
                                            action="{{ route('business.subscription.plans.subscribe', $subscription->plan->id) }}"
                                            method="POST">
                                            @csrf
                                            <button class="btn btn-primary btn-md px-4 action-confirm">
                                    ...
                                    @else
                                        <a href="{{ route('business.subscription.plans.index') }}"
                                            class="btn btn-primary btn-md px-4"><i
                            ...
                            <div class="col">
                                <form action="{{ route('business.subscription.cancel') }}" method="POST">
                                    @csrf
                                    <button class="btn btn-danger btn-md px-4 action-confirm">
                    ...
                    <div class="mb-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="200px" height="200px"
                            ...
                            <path
                                ...
                                transform="d_trans(-290.06582 -168.98458)" fill="#fff" />
                            <path d="M877.7568,217.21468H311.02633a1,1,0,0,1,0-2H877.7568a1,1,0,0,1,0,2Z"
                                transform="d_trans(-290.06582 -168.98458)" fill="#e6e6e6" />
                            ...
                            <path
                                ...
                                transform="d_trans(-290.06582 -168.98458)" fill="#f2f2f2" />
                            <path
                            ...
                            <path
                                ...
                                transform="d_trans(-290.06582 -168.98458)" fill="#f2f2f2" />
                            <polygon points="430.582 548.714 419.933 548.713 414.868 507.64 430.584 507.641 430.582 548.714"
                                fill="#a0616a" />
                            <path
                                ...
                                transform="d_trans(-290.06582 -168.98458)" fill="#2f2e41" />
                            <polygon points="537.725 548.714 527.077 548.713 522.011 507.64 537.728 507.641 537.725 548.714"
                                fill="#a0616a" />
                            <path
                            ...
                            <path
                                d="M683.16043,537.60686a8.7959,8.7959,0,0,0,2.66491-13.12821l21.10689-38.4259-9.19319-8.45131-24.73528,45.82233a8.78357,8.78357,0,0,0,10.15667,14.18309Z"
                                transform="d_trans(-290.06582 -168.98458)" fill="#a0616a" />
                            <path
                                d="M734.6398,357.91347l-4.91986,88.1348L706.01713,501.435l-15.42727,20.45876,1.75369-9.27062-10.63419-5.32074-.99855-.49563,20.41561-79.228,1.88048-30.78742A38.36336,38.36336,0,0,1,717.10322,369.364l11.80767-9.59191Z"
                                transform="d_trans(-290.06582 -168.98458)" fill="#e6e6e6" />
                            <rect x="213.68198" y="191.96732" width="280.48127" height="170.59048" rx="26.74409"
                                fill="#fff" />
                            <path
                                ...
                                transform="d_trans(-290.06582 -168.98458)" fill="#3f3d56" />
                            <rect x="243.92262" y="316.26256" width="60" height="7" rx="3.5"
                                fill="{{ config('theme.settings.colors.primary_color') }}" />
                            ...
                            <path
                                ...
                                transform="d_trans(-290.06582 -168.98458)" fill="#ccc" />
                            <path
                            ...
                            <path
                                ...
                                transform="d_trans(-290.06582 -168.98458)"
                                fill="{{ config('theme.settings.colors.primary_color') }}" />
                        </svg>
                    </div>
                    <h4>{{ d_trans('You are not subscribed') }}</h4>
                    <div class="col-lg-6 m-auto">
                        <p class="mb-3">
                            {{ d_trans('You are not subscribed to any plan, you can subscribe by clicking on the button below') }}
                        </p>
                        <a href="{{ route('business.subscription.plans.index') }}"
                            class="btn btn-outline-primary px-4">{{ d_trans('Subscribe') }}</a>
                    </div>
                </div>
            @endif
        </div>
    ...
    <div class="dashboard-table-container">
        <h4 class="mb-3">{{ d_trans('Transactions') }}</h4>
                ...
                <div class="table-search">
                    <form action="{{ url()->current() }}" method="GET">
                        <div class="row g-3">
                            ...
                            <div class="col-lg-2">
                                <select name="status" class="selectpicker selectpicker-md"
                                    title="{{ d_trans('Status') }}">
                    ...
                    <table class="table align-middle">
                        ...
                        <tbody>
                            @forelse ($transactions as $trx)
                                <tr>
                                    <td>
                                        <a href="{{ route('business.subscription.transactions.show', $trx->id) }}">
                                    ...
                                    <td class="text-end">
                                        <div class="row row-cols-auto justify-content-center align-items-center g-2">
                                            <div class="col">
                                                @if ($trx->isPaid())
                                                    <a href="{{ route('business.subscription.transactions.invoice', $trx->id) }}"
                            ...
                            @empty
                                @include('themes.basic.business.partials.empty-table', ['colspan' => 9])
                            @endforelse
                        </tbody>
                    </table>
                </div>
