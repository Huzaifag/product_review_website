...
@section('title', d_trans('Verify Your Email Address'))
@section('content')
    <div class="mb-3">
        <svg xmlns="http://www.w3.org/2000/svg" width="170" height="170" viewBox="0 0 570 511.67482"
            ...
            <path
                ...
                transform="translate(-315 -194.16259)" fill="{{ config('theme.settings.colors.primary_color') }}" />
            <path
    ...
    <div>
        <p class="h3 mb-3">{{ d_trans('Verify Your Email Address') }}</p>
        <p class="mb-3">
            {{ d_trans("Please verify your email, simply click on the verification link that has been sent to your email address. If you haven't received the verification email, please check your spam or junk folder, or request a new verification email to be sent.") }}
        </p>
        <div class="row justify-content-center g-3">
            <div class="col-12 col-lg">
                <form class="d-inline" method="POST" action="{{ route('business.verification.resend') }}">
                    @csrf
                    <button class="btn btn-primary btn-md w-100">{{ d_trans('Resend') }}</button>
                </form>
            </div>
            <div class="col-12 col-lg">
                <button class="btn btn-outline-primary btn-md w-100" data-bs-toggle="modal"
                    data-bs-target="#emailModal">{{ d_trans('Change Email') }}</button>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content p-4">
                <div class="modal-header border-0 p-0 mb-3">
                    <h5 class="modal-title" id="createFolderModalLabel">{{ d_trans('Email address') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('business.change.email') }}" method="POST">
                    @csrf
                    <div class="modal-body p-0">
                        <div class="mb-3">
                            <input type="email" name="email" id="email" class="form-control form-control-md"
                                placeholder="{{ d_trans('Email address') }}" value="{{ authUser()->email }}" required>
                        </div>
                        <div class="row justify-content-center g-3">
                            <div class="col-12 col-lg">
