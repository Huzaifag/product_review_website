...
@section('title', d_trans('User'))
@section('header_title', d_trans('User Settings'))
@section('back', route('admin.settings.index'))
@section('form', true)
@section('content')
    <form id="submittedForm" action="{{ route('admin.settings.user.update') }}" method="POST">
        @csrf
        <div class="row g-4">
            <div class="col-12">
                <div class="card">
                        ...
                        <div class="row row-cols-1 row-cols-lg-2 row-cols-xl-2 g-3">
                            @foreach (config('settings.user.actions') as $key => $value)
                                <div class="col">
                                    ...
                                    <input type="checkbox" name="user[actions][{{ $key }}]" data-toggle="toggle"
