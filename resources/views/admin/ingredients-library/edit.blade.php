...
@section('content')
    <div class="card">
        <div class="card-body p-4">
            <form id="submittedForm" action="{{ route('admin.ingredients-library.update', $ingredient->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        ...
                        <select name="severity" class="form-select form-select-md">
                            ...
                            <option value="avoid" {{ old('severity', $ingredient->severity) === 'avoid' ? 'selected' : '' }}>{{ d_trans('Avoid') }}</option>
                            <option value="concern" {{ old('severity', $ingredient->severity) === 'concern' ? 'selected' : '' }}>{{ d_trans('Concern') }}</option>
                            <option value="caution" {{ old('severity', $ingredient->severity) === 'caution' ? 'selected' : '' }}>{{ d_trans('Caution') }}</option>
                        </select>
                        @error('severity')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="concern_description" class="form-control" rows="4">{{ old('concern_description', $ingredient->concern_description) }}</textarea>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="health_effects" class="form-control" rows="4">{{ old('health_effects', $ingredient->health_effects) }}</textarea>
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" name="regulatory_status" class="form-control form-control-md"
                            value="{{ old('regulatory_status', $ingredient->regulatory_status) }}" />
                    ...
                    <div class="col-12">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" name="is_published" value="1" id="is_published"
                                {{ old('is_published', $ingredient->is_published) ? 'checked' : '' }} />
                    ...
                    <div class="col-12">
                        <button type="submit" class="btn btn-primary">{{ d_trans('Update') }}</button>
                    </div>
                </div>
            </form>
        </div>
