...
class InstallController extends Controller
{
    ...
    public function licenseAction(Request $request)
    {
        ...
        //     return redirect()->back()->withErrors([d_trans('Purchase code is required')]);
        // }
        ...
        //         return redirect()->back()->withErrors([d_trans('Invalid purchase code')]);
        //     }
        // }
        if (isInLiveServer()) {
            try {
                ...
                if (isset($response->status)) {
                    ...
                    return redirect()->back()->withErrors([d_trans('Connection error, please try again later')]);
                }
            } catch (Exception $e) {
                return redirect()->back()->withErrors([d_trans('Connection error, please try again later')]);
            }
    ...
    public function databaseDetailsAction(Request $request)
    {
        ...
        if (str_contains($request->db_host, '#') || str_contains($request->db_name, '#') || str_contains($request->db_user, '#')) {
            return redirect()->back()->withErrors([d_trans('Database details cannot contain a hashtag #')])->withInput();
        }

        try {
            if (!function_exists('curl_version')) {
                return redirect()->back()->withErrors([d_trans('CURL does not exist in server')])->withInput();
            }
            if (!is_writable(base_path('.env'))) {
                return redirect()->back()->withErrors([d_trans('.env file is not writable')])->withInput();
            }
            if (!@mysqli_connect($request->db_host, $request->db_user, $request->db_pass, $request->db_name)) {
                return redirect()->back()->withErrors([d_trans('Incorrect database details')])->withInput();
            }
    ...
    public function databaseImportAction(Request $request)
    {
        if (!file_exists(base_path('database/sql/data.sql'))) {
            return redirect()->back()->withErrors([d_trans('SQL file is missing') . ' ' . base_path('database/sql/data.sql')])->withInput();
        }
        try {
            ...
            if (DB::connection()->getDatabaseName()) {
                ...
                return redirect()->back()->withErrors([d_trans('Could not find the database. Please check your configuration.')]);
            }
        } catch (Exception $e) {
    ...
    public function completeAction(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'business_path' => ['required', 'string', 'alpha_num'],
            'username' => ['required', 'string', 'min:3', 'max:50', 'alpha_dash', 'unique:admins'],
        ...
        if (str_contains($request->website_url, '#')) {
            return redirect()->back()->withErrors([d_trans('Website URL cannot contain a hashtag #')])->withInput();
        }

        $createAdmin = Admin::create([
            'firstname' => $request->website_name,
            'lastname' => 'Admin',
