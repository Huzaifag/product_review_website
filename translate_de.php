...
// Fetch only untranslated de rows (value = key)
$rows = $pdo->query("SELECT id, `key`, `value` FROM translates WHERE lang='de' AND `value` = `key`")->fetchAll(PDO::FETCH_ASSOC);
...
foreach ($rows as $i => $row) {
    ...
    $translated = googleTranslate($protected, 'de');
